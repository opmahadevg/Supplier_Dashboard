...
export default function MatchedRFQs() {
  ...
  useEffect(() => {
    getRFQs().then(data => { if (data) setRfqList(data) })
  }, [])
...
function BidDrawer({ rfq, isSaved, bid, onClose, onToggleSave, onDecline, onMarkReviewed, onBidPlaced, navigate }) {
  const [tab, setTab] = useState(rfq.status === 'bid_placed' ? 'details' : 'bid')
  const [bidAmount, setBidAmount] = useState(bid?.amount || '')
  const [deliveryDays, setDeliveryDays] = useState(bid?.days || '')
  ...
  const validate = () => {
    const e = {}
    if (!bidAmount || isNaN(Number(bidAmount)) || Number(bidAmount) <= 0) e.amount = 'Enter a valid quote amount'
    if (!deliveryDays || isNaN(Number(deliveryDays)) || Number(deliveryDays) <= 0) e.days = 'Enter delivery days'
    return e
  }
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      setSubmitted(true)
      onBidPlaced({ amount: bidAmount, days: deliveryDays, note, validUntil })
    }, 900)
  }
    ...
    return (
      <div className="h-full flex items-center justify-center p-8 min-h-[400px]">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-[#111111] mb-2">Bid Submitted!</h2>
          <p className="text-sm text-[#9e9e9e] mb-1">Your quote of <strong>${bidAmount}</strong> has been sent to {rfq.buyer}.</p>
...
function BidPanel({ rfq, bidAmount, setBidAmount, deliveryDays, setDeliveryDays, note, setNote, validUntil, setValidUntil, errors, setErrors, submitting, alreadyBid, bid, onSubmit, navigate }) {
  const totalValue = bidAmount && !isNaN(bidAmount) ? Number(bidAmount).toLocaleString() : '—'
  const withinBudget = bidAmount && Number(bidAmount) >= rfq.budgetMin && Number(bidAmount) <= rfq.budgetMin * 1.5
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div>
        <label className="text-xs font-medium text-[#555555] block mb-1.5">
          Your Total Quote (USD) <span className="text-[#ba1a1a]">*</span>
        </label>
        <div className={`flex items-center border rounded-xl overflow-hidden focus-within:border-[#0f00da] transition-colors ${errors.amount ? 'border-[#ba1a1a]' : 'border-[#ebebeb]'}`}>
          <span className="px-3 text-[#9e9e9e] text-sm bg-[#f7f7f7] h-11 flex items-center border-r border-[#ebebeb]">$</span>
          <input
            type="number"
            value={bidAmount}
            onChange={e => { setBidAmount(e.target.value); setErrors(er => ({ ...er, amount: '' })) }}
            placeholder="0"
            min="0"
            className="flex-1 px-3 py-2.5 text-sm outline-none bg-white"
          />
        </div>
        {errors.amount && <p className="text-xs text-[#ba1a1a] mt-1 flex items-center gap-1"><span className="material-symbols-outlined text-[12px]">error</span>{errors.amount}</p>}
        {bidAmount && (
          <p className={`text-xs mt-1 flex items-center gap-1 ${withinBudget ? 'text-[#2e7d32]' : 'text-[#e65100]'}`}>
            <span className="material-symbols-outlined text-[12px]">{withinBudget ? 'check_circle' : 'info'}</span>
            {withinBudget ? 'Within buyer budget range' : 'Outside buyer budget range — add a note to justify'}
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs font-medium text-[#555555] block mb-1.5">
            Delivery Days <span className="text-[#ba1a1a]">*</span>
          </label>
          <div className="flex flex-wrap gap-1 mb-1.5">
            {['7', '14', '21', '30'].map(d => (
              <button
                key={d}
                type="button"
                onClick={() => { setDeliveryDays(d); setErrors(er => ({ ...er, days: '' })) }}
                className={`px-2.5 py-1 rounded-full text-xs border transition-colors ${deliveryDays === d ? 'bg-[#0f00da] text-white border-[#0f00da]' : 'border-[#ebebeb] text-[#555555] hover:border-[#0f00da]'}`}
              >
                {d}d
              </button>
            ))}
          ...
          <input
            type="number"
            value={deliveryDays}
            onChange={e => { setDeliveryDays(e.target.value); setErrors(er => ({ ...er, days: '' })) }}
            placeholder="Custom days"
            min="1"
            className={`w-full border rounded-xl px-3 py-2 text-sm outline-none focus:border-[#0f00da] ${errors.days ? 'border-[#ba1a1a]' : 'border-[#ebebeb]'}`}
          />
          {errors.days && <p className="text-xs text-[#ba1a1a] mt-1 flex items-center gap-1"><span className="material-symbols-outlined text-[12px]">error</span>{errors.days}</p>}
        </div>
        <div>
          ...
          <input
            ...
            className="w-full border border-[#ebebeb] rounded-xl px-3 py-2 text-sm outline-none focus:border-[#0f00da] mt-[calc(1.5rem+4px)]"
      ...
      <div>
        ...
        <textarea
          ...
          rows={3}
          placeholder="Certifications, payment terms, special conditions, or why you're the best fit..."
      ...
      {bidAmount && deliveryDays && (
        <div className="bg-[#e1e0ff] rounded-xl p-4">
          <p className="text-xs font-semibold text-[#0f00da] mb-2">Bid Summary</p>
          <div className="grid grid-cols-3 gap-2 text-xs">
            <div><p className="text-[#555555] opacity-70">Quote</p><p className="font-bold text-[#0f00da]">${totalValue}</p></div>
            <div><p className="text-[#555555] opacity-70">Delivery</p><p className="font-bold text-[#0f00da]">{deliveryDays} days</p></div>
            <div><p className="text-[#555555] opacity-70">Your Match</p><p className="font-bold text-[#0f00da]">{rfq.match}%</p></div>
          </div>
        </div>
      )}
